...
class DETRVAE(nn.Module):
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, instr_embedding=None):
        """
        qpos: batch, qpos_dim
        ...
        if self.backbones is not None:
            ...
            # Compute projection of instruction embedding
            if instr_embedding is not None:
                instr_embedding = self.instr_embedding_proj(instr_embedding.unsqueeze(0))

            # fold camera dimension into width dimension
            src = torch.cat(all_cam_features, axis=3)
            pos = torch.cat(all_cam_pos, axis=3)
            hs = self.transformer(src, None, self.query_embed.weight, pos, latent_input, proprio_input, self.additional_pos_embed.weight, instr_embedding=instr_embedding)[0]
        else:
